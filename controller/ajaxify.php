				...
				else
				{
					...
					else
					{
						if (!$row['timestamp'])
						{
							...
							$sql = 'INSERT INTO ' . $this->table_prefix . 'posts_likes (post_id, user_id, type, timestamp) VALUES (' . (int) $post . ', ' . $this->user->data['user_id'] . ', \'post\', ' . time() . ')';
							$result = $this->db->sql_query($sql);
							$this->db->sql_freeresult($result);
							...
							return new \Symfony\Component\HttpFoundation\JsonResponse(array(
								'toggle_action'	=> 'add',
								'toggle_post'	=> (int) $post,
							));
						}
						else
						{
							//so we have a record ... and the user don't love it anymore!
							$sql = 'DELETE FROM ' . $this->table_prefix . 'posts_likes WHERE post_id = ' . (int) $post . ' AND user_id = ' . $this->user->data['user_id'];
							$result = $this->db->sql_query($sql);
							$this->db->sql_freeresult($result);

...
class postoftheday_listener implements EventSubscriberInterface
{
	...
	public function  potd_index_page( $event)
	{
		...
		$now = time();
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_index_most_liked_today'], $now - SECONDS_PER_DAY, 'LIKES_TODAY');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_index_most_liked_this_week'], $now - SECONDS_PER_DAY * 7, 'LIKES_THIS_WEEK');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_index_most_liked_this_month'], $now - SECONDS_PER_DAY * 31, 'LIKES_THIS_MONTH');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_index_most_liked_this_year'], $now - SECONDS_PER_DAY * 366, 'LIKES_THIS_YEAR');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_index_most_liked_ever'], 2, 'LIKES_EVER');
	...
	{ 
		...
		// build the template array of most liked posts
		$now = time();
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_forum_most_liked_today'], $now - SECONDS_PER_DAY, 'LIKES_TODAY');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_forum_most_liked_this_week'], $now - SECONDS_PER_DAY * 7, 'LIKES_THIS_WEEK');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_forum_most_liked_this_month'], $now - SECONDS_PER_DAY * 31, 'LIKES_THIS_MONTH');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_forum_most_liked_this_year'], $now - SECONDS_PER_DAY * 366, 'LIKES_THIS_YEAR');
		$rows += $this->topposts_of_period($forum_ary, $this->config['postlove_forum_most_liked_ever'], 2, 'LIKES_EVER');
	...
	function topposts_of_period($forum_ary, $howmany, $time_threshold, $period_name)
	{
